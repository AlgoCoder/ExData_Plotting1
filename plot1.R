global_active_power_hist <- function(file = stop(" 'file' must be specified"), toDate = "2007-02-01", fromDate = "2007-02-02") {
  ...
  if(file.access(file, 4) == 0) {
      ...
      png(file = "plot1.png")
      with(extract.df, hist(Global_active_power, main = "Global Active Power", xlab = "Global Active Power (kilowatts)", ylab = "Frequency", col = "red", ylim = c(0, 1200)))
      dev.off()
