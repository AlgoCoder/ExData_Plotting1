multiple_power_params <- function(file = stop(" 'file' must be specified"), toDate = "2007-02-01", fromDate = "2007-02-02") {
  ...
  if(file.access(file, 4) == 0) {
      ...
      with(extract.df, {
        ...
        plot(x = Date, y = Global_active_power, type = "l", ylab = "Global Active Power", xlab = "")
        
        plot(x = Date, y = Voltage, xlab = "datetime", ylab = "Voltage", type = "l")
        
        {
        
            plot (x = Date, y = Sub_metering_1, type = "l", col = "black", ylab = "Energy sub metering", xlab = "")
        ...
        plot(x = Date, y = Global_reactive_power, type = "l", ylab = "Global Reactive Power", xlab = "datetime")
